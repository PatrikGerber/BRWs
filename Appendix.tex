...
% \subsection{Stable distributions}\label{subsec:stable_distributions}
% The material covered here is mainly taken from \cite[Section XVII]{feller1957introduction} and we will refer to distributions and their characteristic function interchangeably. Let $\omega$ be the characteristic function of some distribution on $\R$. $\omega$ is called \textit{infinitely divisible} if for all $n \in \N$ there exists a characteristic function $\omega_n$ such that 
% \begin{equation}
% \omega_n^n = \omega. 
% \end{equation}
...
% \begin{equation}
% \left[ \phi\left( \frac{t}{a_n}\right) e^{-itb_n} \right] ^n \to \omega(t)\qquad\forall\,t \in \R. 
% \end{equation}
